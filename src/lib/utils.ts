...
export const fixThumbnailUrl = (url: string | undefined): string => {
  ...
  if (url.startsWith('https://ytimgf.youtube-anime.com/images/')) {
    finalUrl = url.replace('https://ytimgf.youtube-anime.com/images/', 'https://wp.youtube-anime.com/aln.youtube-anime.com/') + '?w=200';
  } else if (url.startsWith('https://cdnimg.xyz')) {
    finalUrl = `https://wp.youtube-anime.com/${url.substring('https://'.length)}?w=200`;
  } else if (url.startsWith('https://aln.youtube-anime.com')) {
    finalUrl = url.replace('https://aln.youtube-anime.com/', 'https://wp.youtube-anime.com/aln.youtube-anime.com/images/') + '?w=200';
  } else if (url.startsWith('__Show__')) {
    finalUrl = `https://wp.youtube-anime.com/aln.youtube-anime.com/images/${url}?w=200`;
    ...
    finalUrl = `/api/image-proxy?url=${encodeURIComponent(url)}&w=200`;
  } else if (url.startsWith('images2')) {
    finalUrl = `https://wp.youtube-anime.com/aln.youtube-anime.com/${url}?w=200`;
  } else {
    finalUrl = `https://wp.youtube-anime.com/aln.youtube-anime.com/images/${url}?w=200`;
  }
