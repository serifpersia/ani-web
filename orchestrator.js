...
const startSpinner = (msg) => {
    ...
    syncSpinner = setInterval(() => {
        ...
        process.stdout.write(`\r${colors.system}[System]${colors.reset} ${msg}${'.'.repeat(syncDots)}${' '.repeat(3 - syncDots)}`);
    }, 400);
};
...
const log = (prefix, color, data) => {
    ...
    if (str.includes('[SERVER_EXIT]')) {
        stopSpinner();
        console.log(`${colors.system}[System]${colors.reset} Server sync complete. Shutting down cleanly.`);
    ...
    if (syncSpinner) {
        ...
        // redraw the spinner
        process.stdout.write(`${colors.system}[System]${colors.reset} ${syncMessage}${'.'.repeat(syncDots)}${' '.repeat(3 - syncDots)}`);
    } else {
        for (const line of lines) {
            console.log(`${color}[${prefix}]${colors.reset} ${line}`);
        }
...
console.log(`${colors.system}[System]${colors.reset} Starting ani-web in ${mode.toUpperCase()} mode...`);
console.log(`${colors.system}[System]${colors.reset} Press 'q' or 'Ctrl+C' to cleanly exit and sync data.\n`);
...
const shutdown = () => {
    ...
    req.on('error', () => {
        ...
        if (isWin && serverProcess) spawn('taskkill', ['/pid', serverProcess.pid, '/f', '/t'], { shell: true });
