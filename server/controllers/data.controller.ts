...
export class DataController {
    ...
    getPopular = async (req: Request, res: Response) => {
        const timeframe = (req.params.timeframe as string).toLowerCase() as 'daily' | 'weekly' | 'monthly' | 'all';
        try {
    ...
    getSkipTimes = async (req: Request, res: Response) => {
        try {
            const data = await this.provider.getSkipTimes(req.params.showId as string, req.params.episodeNumber as string);
            res.json(data);
        } catch {
    ...
    getVideo = async (req: Request, res: Response) => {
        try {
            res.json(await this.provider.getStreamUrls(req.query.showId as string, req.query.episodeNumber as string, req.query.mode as any));
        } catch {
            res.status(500).send('Error');
        }
    };
