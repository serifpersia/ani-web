...
async function getRemoteVersion(remoteFolder: string): Promise<{ version: number, fileId?: string }> {
    try {
        ...
        if (activeProvider === 'rclone') {
            ...
            await rcloneService.downloadFile(remoteFolder, CONFIG.MANIFEST_FILENAME, CONFIG.TEMP_MANIFEST_PATH);
            const content = await fs.readFile(CONFIG.TEMP_MANIFEST_PATH, 'utf-8');
            await fs.unlink(CONFIG.TEMP_MANIFEST_PATH);
...
async function getSyncMetadata(db: Database): Promise<{ localVersion: number, isDirty: boolean }> {
    return new Promise((resolve, reject) => {
        db.all('SELECT key, value FROM sync_metadata', (err: Error | null, rows: { key: string, value: number }[]) => {
...
export async function syncDownOnBoot(db: Database, dbPath: string, remoteFolderName: string, closeMainDb: () => Promise<void>): Promise<boolean> {
...
export async function syncUp(db: Database, dbPath: string, remoteFolderName: string): Promise<void> {
    ...
    try {
        ...
        if (activeProvider === 'google') {
            ...
            await googleDriveService.uploadFile(dbPath, dbName, 'application/x-sqlite3', folderId, remoteDbFile?.id);
            await googleDriveService.uploadFile(CONFIG.TEMP_MANIFEST_PATH, CONFIG.MANIFEST_FILENAME, 'application/json', folderId, manifestId);

        } else if (activeProvider === 'rclone') {
            await rcloneService.uploadFile(dbPath, remoteFolderName, dbName);
            await rcloneService.uploadFile(CONFIG.TEMP_MANIFEST_PATH, remoteFolderName, CONFIG.MANIFEST_FILENAME);
        }
        ...
        await new Promise<void>((resolve, reject) => {
            db.serialize(() => {
                ...
                db.run('UPDATE sync_metadata SET value = ? WHERE key = "last_synced_version"', [localVersion], (err: Error | null) => {
...
export function initializeDatabase(dbPath: string): Promise<Database> {
    return new Promise((resolve, reject) => {
        const db = new sqlite3.Database(dbPath, (err: Error | null) => {
            if (err) { log.error({ err }, 'Database opening error'); return reject(err); }
        });
        db.configure('busyTimeout', 5000);
        db.serialize(() => {
