...
const Search: React.FC = () => {
    ...
    const { data, fetchNextPage, hasNextPage, isFetchingNextPage, isLoading, isError, error } = useSearchAnime(searchParams.toString());
    ...
    return (
        ...
        <button className="btn-primary" style={{alignSelf: 'flex-start'}} onClick={handleSearch}>
        Apply Filters
        </button>
        </div>
        </div>
        ...
        {!isLoading && results.length === 0 && <p style={{textAlign:'center', color:'var(--text-secondary)'}}>No results found.</p>}
        </div>
    );
};
